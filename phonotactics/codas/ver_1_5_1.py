...
from coventreiya.phonology.consonants import ver_1_5_4 as cons_ver

class ver_1_5_1( Codas ):
    def __init__(self):
        min_length = 1
        max_length = 5
...
cod_ = register( ver_1_5_1(), gen_ver_1_5_1 )

##################################################################################
#
#    The OLD *DEPRECEATED* "default" gen_*_ functions ...
#     The Backwards Compatability Functions
#        Old Actual Codas Generation Functions - Here for Backward Compatability
#      
#

def gen_codas():
    return cod_.all_allowable_sets_()

def gen_coda_c1():
    return cod_.categories()[0]

def gen_coda_c2():
    return cod_.categories()[1]

def gen_actual_():
    """ WARNING!!! this fuction may use A LOT of memory """
    """  and may crash python!!!"""
    return cod_.all_actuals()
    
def gen_actual_file_(path, encoding_='utf-8'):
    return cod_.all_actuals_to_file(path, encoding_)
